...
namespace Microsoft.Maps.SpatialToolbox
{
    ...
    public static class SpatialTools
    {
        ...
        public static double CalculateHeading(Coordinate origin, Coordinate destination)
        {
            ...
            double dx = Math.Cos(radianLat1)*Math.Sin(radianLat2) -
        ...
        public static Coordinate CalculateDestinationCoordinate(Coordinate origin, double bearing, double distance,
            DistanceUnits units)
        {
            ...
            double destinationLatitudeRad =
                Math.Asin(Math.Sin(latitudeRad)*Math.Cos(centralAngle) +
                          Math.Cos(latitudeRad)*Math.Sin(centralAngle)*Math.Cos(bearingRad));
            double destinationLongitudeRad = longitudeRad +
                                             Math.Atan2(
                                                 Math.Sin(bearingRad)*Math.Sin(centralAngle)*Math.Cos(latitudeRad),
                                                 Math.Cos(centralAngle) -
                                                 Math.Sin(latitudeRad)*Math.Sin(destinationLatitudeRad));
        ...
        public static double HaversineDistance(Coordinate origin, Coordinate destination, DistanceUnits units)
        {
            ...
            double a = Math.Pow(Math.Sin(dLat/2), 2) +
                       Math.Pow(Math.Cos(ToRadians(origin.Latitude)), 2)*Math.Pow(Math.Sin(dLon/2), 2);
        ...
        public static CoordinateCollection GenerateRegularPolygon(Coordinate center, double radius, DistanceUnits units,
            int numberOfPoints, double offset)
        {
        ...
        {
            return Task.Run<CoordinateCollection>(() => { return VertexReduction(coordinates, tolerance); });
        }
        ...
        public static CoordinateCollection VertexReduction(CoordinateCollection coordinates, double tolerance)
        {
            ...
            for (int i = 1; i < coordinates.Count - 1; i++)
            {
                ...
                if (
                    Math.Sqrt(Math.Pow(coordinates[i].Latitude - baseCoord.Latitude, 2) +
                              Math.Pow(coordinates[i].Longitude - baseCoord.Longitude, 2)) > tolerance)
                {
